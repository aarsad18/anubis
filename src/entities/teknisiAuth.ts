import { Model, Table, Column, DataType, Index, Sequelize, HasOne } from "sequelize-typescript";
import { teknisiData } from "./teknisiData";

interface teknisiAuthAttributes {
  id?: string;
...
@Table({ tableName: "teknisi_auth", timestamps: false })
export class teknisiAuth extends Model<teknisiAuthAttributes, teknisiAuthAttributes> implements teknisiAuthAttributes {
  @Column({ primaryKey: true, type: DataType.STRING(36), allowNull: false })
  @Index({ name: "teknisi_auth_pkey", using: "btree", unique: true })
  id?: string;
  @Column({ field: "teknisi_data_id", allowNull: false, type: DataType.STRING })
  teknisiDataId?: string;
  @Column({ allowNull: false, type: DataType.STRING })
  @Index({ name: "teknisi_auth_email_key", using: "btree", unique: true })
  email?: string;
  @Column({ allowNull: false, type: DataType.STRING })
  @Index({ name: "teknisi_auth_phone_key", using: "btree", unique: true })
  phone?: string;
  @Column({ field: "user_name", allowNull: false, type: DataType.STRING })
  @Index({ name: "teknisi_auth_user_name_key", using: "btree", unique: true })
  userName?: string;
  @Column({ allowNull: false, type: DataType.STRING })
  password?: string;
  ...
  updateBy?: string;
  @Column({ field: "player_ids", allowNull: true, type: DataType.STRING })
  playerIds?: string;

  @HasOne(returns => teknisiData, { foreignKey: 'id', sourceKey: 'teknisiDataId' })
